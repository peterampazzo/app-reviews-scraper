...
class GoogleClient(): 
    ...
    def get_details(self) -> dict:
        ...
        return google_play_scraper.app(self.app_id, lang=self.language, 
                                        country=PLAY_STORE_COUNTRY)   

    def get_similar(self) -> list:
        """Return similar apps as list of IDs"""

        return play_scraper.similar(self.app_id, hl=self.language, gl=PLAY_STORE_COUNTRY)  

    def get_reviews(self) -> list:
        """Return app's reviews as JSON"""
        return google_play_scraper.reviews_all(self.app_id,
                sleep_milliseconds=config.get("app.sleep.play_store"),
                lang=self.language, country=PLAY_STORE_COUNTRY)
...
class AppleClient():
    ...
    def __init__(self, app_name: str, app_id: int, country: str):
        ...
        self.client = app_store_scraper.AppStore(app_name=app_name, app_id=app_id, country=country)
    ...
    def get_reviews(self) -> list:
        ...
        self.client.review(how_many=20, retry_after=10, sleep=config.get("app.sleep.apple_store")) 
