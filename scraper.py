...
def run(app_list: str, store: str, reviews: bool=False, 
        details: bool=False, similar: bool=False, refresh_weeks: float=1.0):
    ...
    for app, locale in iterpairs:
        ...
        try: 
            ...
            if store == 'app': 
                client = AppleClient(app_name=app["name"], app_id=app["id"], country=locale)
                app_dirname = app['id']
            else: 
...
if __name__ == '__main__': 
    ...
    parser.add_argument("--list", type=str, dest='app_list', required=True,
                        help="path to JSON file containing apps to query")
    parser.add_argument("--store", type=str, choices=['app', 'play'], required=True, 
                        help='which store to query, app (Apple) or play (Google)')
    parser.add_argument("--reviews", action="store_true", 
                        help="fetch app reviews")
    parser.add_argument("--details", action="store_true", 
                        help="fetch app details")
    parser.add_argument("--similar", action="store_true", 
                        help="fetch similar apps")
    parser.add_argument("--refresh", type=float, dest='refresh_weeks', default=1.0, 
